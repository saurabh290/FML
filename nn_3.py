...
class Net(object):
  ...
  def __init__(self, num_layers, num_units):
    ...
    weights: [(NUM_FEATS, num_units), (num_units, num_units),
    ...
    for i in range(num_layers):
      ...
      if i == 0:
        # Input layer
        self.weights.append(
            np.random.uniform(-1, 1, size=(NUM_FEATS, self.num_units)))
      else:
        # Hidden layer
        self.weights.append(
            np.random.uniform(-1, 1, size=(self.num_units, self.num_units)))
    ...
    for i in range(num_layers):
      ...
      if i==0:
        # Input layer
        self.weights.append(np.sqrt(2/NUM_FEATS) * \
                            np.random.randn(NUM_FEATS, self.num_units))
      else:
        # Hidden layer
        self.weights.append( np.sqrt(2/self.num_units) * \
                            np.random.randn(self.num_units, self.num_units))

      self.gammas.append(np.sqrt(2/self.num_units) * \
                        np.random.randn(self.num_units, 1))
      self.betas.append(np.sqrt(2/self.num_units) * \
                        np.random.randn(self.num_units, 1))
  ...
  def backward(self, X, y, lamda):
    ...
    for i in range(self.num_layers + 1):
      if i == 0:
        ...
        d_h_states.insert(
            0, d_b_states[0] * (self.gammas[index].T/self.std_states[index]))
      d_weights.insert(0,  ((1/batch_size) * np.dot(self.a_states[index].T, d_h_states[0])) + lamda * (
          self.weights[index]))  # +  lamda * self.weights[index]
      d_gammas.insert(
          0, ((1/batch_size) * np.sum(d_b_states[0] * self.b_states[index], axis=0)))
...
class Optimizer(object):
  ...
  def step(self, weights, gammas, betas, delta_weights, delta_gammas, delta_betas):
    ...
    if self.optimizer_type == "SGD":
      ...
      for index in range(len(weights)):
        self.Vt[index] = self.beta * self.Vt[index] + \
            (1 - self.beta) * delta_weights[index]
        self.Bt[index] = self.beta * self.Bt[index] + \
            (1 - self.beta) * delta_betas[index]
        self.Gt[index] = self.beta * self.Gt[index] + \
            (1 - self.beta) * delta_gammas[index]
      ...
      for index in range(len(weights)):
        self.Vt[index] = self.beta * self.Vt[index] + \
            (1 - self.beta) * np.square(delta_weights[index])
        self.Bt[index] = self.beta * self.Bt[index] + \
            (1 - self.beta) * np.square(delta_betas[index])
        self.Gt[index] = self.beta * self.Gt[index] + \
            (1 - self.beta) * np.square(delta_gammas[index])
        weights[index] = weights[index] - (self.learning_rate / (
            np.sqrt(self.Vt[index] + 0.00000001))) * (delta_weights[index])
        betas[index] = betas[index] - \
            (self.learning_rate /
            (np.sqrt(self.Bt[index] + 0.00000001))) * (delta_betas[index])
        gammas[index] = gammas[index] - \
            (self.learning_rate /
            (np.sqrt(self.Gt[index] + 0.00000001))) * (delta_gammas[index])
      # print(self.Vt)
      # print(self.Bt)

    elif self.optimizer_type == "ADAM":
      for index in range(len(weights)):
        self.Vt[index] = self.beta1 * self.Vt[index] + \
            (1 - self.beta1) * delta_weights[index]
        self.Bt[index] = self.beta1 * self.Bt[index] + \
            (1 - self.beta1) * delta_betas[index]
        self.Gt[index] = self.beta1 * self.Gt[index] + \
            (1 - self.beta1) * delta_gammas[index]

        self.Vt2[index] = self.beta2 * self.Vt2[index] + \
            (1 - self.beta2) * np.square(delta_weights[index])
        self.Bt2[index] = self.beta2 * self.Bt2[index] + \
            (1 - self.beta2) * np.square(delta_betas[index])
        self.Gt2[index] = self.beta2 * self.Gt2[index] + \
            (1 - self.beta2) * np.square(delta_gammas[index])

        weights[index] = weights[index] - self.learning_rate * \
            (self.Vt[index] / (np.sqrt(self.Vt2[index] + 0.00000001)))
        betas[index] = betas[index] - self.learning_rate * \
            (self.Bt[index]/(np.sqrt(self.Bt2[index] + 0.00000001)))
        gammas[index] = gammas[index] - self.learning_rate * \
            (self.Gt[index]/(np.sqrt(self.Gt2[index] + 0.00000001)))
...
def loss_regularization(weights, gammas, betas):
  ...
  for i in range(len(weights)):
    loss = loss + np.sum(np.square(weights[i])) + np.sum(
        np.square(gammas[i])) + np.sum(np.square(betas[i]))
  return loss
...
def loss_fn(y, y_hat, weights, gammas, betas, lamda):
  ...
  ----------
    l2 regularization loss
  '''
  loss = loss_mse(y, y_hat) + lamda * \
                  loss_regularization(weights, gammas, betas)
  return loss
...
def train(
  ...
  for e in range(max_epochs):
    ...
    for i in range(0, m, batch_size):
      ...
      weights_updated, gammas_updated, betas_updated = optimizer.step(
          net.weights, net.gammas, net.betas, dW, dgammas, dbetas)
      ...
      batch_loss = loss_fn(batch_target, pred, net.weights,
                          net.gammas, net.betas, lamda)
...
def scratch_pca(matrix, n_components, set, u):
  mean_matrix = matrix - np.mean(matrix, axis=0)
  covar = np.cov(mean_matrix, rowvar=False)
  ret = np.linalg.eigh(covar)
  eig_val = ret[0]
  eig_vec = ret[1]
  ...
  if set == "train":
    for i in sorted(eig_val, reverse=True)[:n_components]:
      ind = list(eig_val).index(i)
      U.append(eig_vec[ind])

  else:
    Z = np.transpose(np.dot(u, np.transpose(matrix)))
    return Z, u

  Z = np.transpose(np.dot(U, np.transpose(matrix)))
  x_app = np.transpose(np.dot(np.transpose(U), np.transpose(Z)))
  var = np.square(matrix - x_app) / np.square(matrix)
...
def read_data():
  ...
  if is_pca == "Y":
    train_input, U = scratch_pca(train_input, n_components, "train", [])
    dev_input, _  = scratch_pca(dev_input, n_components, "dev", U)
    test_input, _  = scratch_pca(test_input, n_components, "test", U)
